...
SELECT AVG(total_customer_mileage) AS average_distance FROM Mileage;
...
SELECT MAX(aircraft_type) AS most_reserved_aircraft FROM Aircraft
                                                             INNER JOIN Flight ON Aircraft.aircraft_id = Flight.aircraft_id
                                                             INNER JOIN Mileage ON Flight.flight_id = Mileage.flight_id
                                                             INNER JOIN Customer ON Mileage.customer_id = Customer.customer_id
                                                             INNER JOIN Status ON Customer.customer_status = Status.status_id
